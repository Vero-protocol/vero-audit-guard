...
pub enum AuditGuardError {
    ...
    #[error("API URL parse error: {0}")]
    InvalidApiUrl(String),

    #[error("Endpoint path error: {0}")]
...
impl AuditGuardClient {
    ...
    fn build_endpoint(&self, path: &str) -> Result<Url> {
        self.api_url
            .join(path)
    ...
    pub fn new_validated(api_url: &str) -> Result<Self> {
        validate_url(api_url)?;
        let parsed_url = Url::parse(api_url.trim())
    ...
    pub async fn submit_report(&self, report: &AuditReport) -> Result<()> {
    ...
    pub async fn get_report(&self, id: &str) -> Result<AuditReport> {
        validate_url(self.api_url.as_str())?;
        if id.trim().is_empty() {
